...
```
Based on the results above, the p-value is larger than the level of significance, so we cannot reject the null hypothesis. The t-statistic is within +/- 2, so we cannot reject the null hypothesis of zero difference. The CI shows values between -7 and and 542, so the difference between the online and offline prices can be zero. Thus, we can conclude that the offline and online prices of pizza margherita can be equal in Budapest.
