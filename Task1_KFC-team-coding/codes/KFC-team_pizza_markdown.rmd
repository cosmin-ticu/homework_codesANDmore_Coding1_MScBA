...
The tables below contain some important summary statistics about the price of pizza margerita (Table 1) and sparkling water (Table 2). 
```{r, '2a' , echo = FALSE, out.width='50%'}
library(moments)
...
knitr::kable(pizza_stat,caption="Table 1: Descriptive statistics for pizza margherita prices in Budapest")
```

```{r , '2b' , echo = FALSE, out.width='50%'}
library(moments)
...
knitr::kable(pizza_stat,caption="Table 2: Descriptive statistics for sparkling water prices of pizzerias in Budapest")
```
...
The hypotheses are about whether the offline versus online price difference is zero on average in population of products represented by the data.
* The null hypothesis is that online and offline prices of pizza margherita are the same in Budapest. 
* The alternative hypothesis is that online and offline prices of pizza margherita are different in Budapest.
We fixed the level of significance at 5% then tested hypotheses.
```{r, '5' , echo=FALSE, fig.cap="Hypothesis Testing" , purl=FALSE}
t.test(price_pizza ~ delivery, data = pizza_data)

```
Based on the results above, the p-value is smaller than the level of significance, so we could reject the null hypothesis. However, the t-statistic is within +/- 2, so we cannot reject the null hypothesis of zero difference. The CI shows values between -7 and and 542, so the difference between the online and offline prices can be zero. Thus, we can conclude that the offline and online prices of pizza margherita can be equal in Budapest.
